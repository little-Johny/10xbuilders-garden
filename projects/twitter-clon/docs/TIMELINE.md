...
| 2026-04-05 | **Fase 5 — Comentarios y likes** | Se implementa la funcionalidad completa de comentarios en tweets: listar, crear (con soporte a hilos via `parent_comment_id`) y dar like. Se añaden también likes en tweets (toggle insert/delete). Nuevos módulos en la API: `comments.routes.js` (rutas anidadas bajo `/tweets/:tweetId/comments` con `mergeParams`), `comment.service.js`, `commentLike.service.js` y `tweetLike.service.js`. En el frontend se crean los componentes `TweetCard` (extraído de FeedPage/ProfilePage para reutilización), `CommentSection` y `CommentCard`. |
| 2026-04-07 | **Mantenimiento** | Corrección de typos y mejora de documentación en skills de Cursor AI (`.cursor/skills/`). Sin cambios funcionales. |

---
*Última actualización: 2026-04-12 — basado en el historial git hasta el commit `4397b80`*
